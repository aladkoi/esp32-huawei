...
#define SPP_SERVER_NAME "BLE_HUAWEI"
#define TAG_BL "SPP"
...
static void send_task(void *pvParameters) {
    ...
    while (1) {
        if (spp_handle != 0) { // Проверяем, подключён ли клиент
                send_buffer[2]=1; 
                send_buffer[3]=(uint8_t)0x15;///  (uint8_t)(state.Vbatt >> 8); //бат
                send_buffer[4]=(uint8_t)0x18; //(uint8_t)(state.Vbatt  & 0xFF); //бат
                send_buffer[5]=(uint8_t)(state.volt_bl >> 8); //erpm
                send_buffer[6]=(uint8_t)(state.volt_bl & 0xFF); //erpm
            ...
            ESP_LOG_BUFFER_HEX(TAG_BL, send_buffer, 9);
